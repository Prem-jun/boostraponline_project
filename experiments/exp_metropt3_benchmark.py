"""
SPC Benchmark Experiment: MetroPT-3 Air Compressor Dataset
============================================================

Evaluates RBULT-SPC (Memory-Bounded Adaptive Control Chart) against
three benchmark SPC baselines on time-series telemetry from MetroPT-3:
...
Monitored Telemetry Channels (D = 7):
  - TP2 (Compressor Pressure [bar])
  - TP3 (Pneumatic Panel Pressure [bar])
  - H1 (Separator Filter Pressure Drop [bar])
  - DV_pressure (Air Dryer Discharge Pressure Drop [bar])
  - Reservoirs (Downstream Reservoir Pressure [bar])
  - Oil_temperature (Compressor Oil Temperature [°C])
  - Motor_current (Three-Phase Motor Current [A])

Ground-Truth Failure Windows (Company Failure Reports):
  - Event 1: 2020-04-18 00:00:00 to 2020-04-18 23:59:50 (Air Leak - High Stress)
...
def load_and_label_metropt3(csv_path: str = 'MetroPT3/MetroPT3_AirCompressor.csv') -> pd.DataFrame:
    """Load MetroPT3 dataset and tag ground-truth failure timestamps."""
    print(f"Loading MetroPT3 dataset: {csv_path}...")
    ...
    if 'timestamp' in df.columns:
        ...
        raise KeyError("Timestamp column not found in dataset!")

    # Define failure time windows from company logs
    failure_windows = [
        (pd.Timestamp('2020-04-18 00:00:00'), pd.Timestamp('2020-04-18 23:59:50')),
        ...
        (pd.Timestamp('2020-07-15 14:30:00'), pd.Timestamp('2020-07-15 19:00:00'))
    ]

    # Create binary Ground Truth failure label (1 = Out-of-Control failure, 0 = In-Control)
    is_failure = pd.Series(0, index=df.index)
    for start, end in failure_windows:
        is_failure |= (df['datetime'] >= start) & (df['datetime'] <= end)
    df['failure_label'] = is_failure.astype(int)

    num_failures = df['failure_label'].sum()
    print(f"Total rows loaded: {len(df):,}")
    print(f"Failure samples: {num_failures:,} ({num_failures / len(df) * 100:.2f}% of stream)")
    return df


def run_metropt3_spc_benchmark(csv_path: str = 'MetroPT3/MetroPT3_AirCompressor.csv',
                               chunk_size: int = 1000,
                               outlier_filter: bool = True,
                               ooc_threshold_count: Optional[int] = None) -> dict:
    """Run streaming SPC benchmark comparing 4 methods on MetroPT-3 dataset.

    Args:
        csv_path: Path to MetroPT3 dataset CSV.
        chunk_size: Streaming chunk size (samples per batch, default 1000).
    ...
    # Monitored analogue telemetry features (D = 7)
    features = [
        'TP2',
        'TP3',
        'H1',
    ...
    print(f"Chunk distribution -> In-Control Chunks: {in_control_chunks:,} | Out-of-Control Failure Chunks: {ooc_chunks:,}")
    ...
    start_time = time.perf_counter()
    shewhart_bounds = {}
    # Use first chunk (Phase 1 clean reference) to set Shewhart 3-sigma limits
    ...
    for i in range(num_chunks):
        ...
        # Accumulate stream history (causes O(N * D) memory expansion)
        for feat in features:
        ...
        # Compute full percentile limits across accumulated history
        for feat in features:
    ...
    print("\n=====================================================================================================================")
    print("                                METRO PT-3 SPC BENCHMARK COMPARISON MATRIX                                            ")
    print("=====================================================================================================================")
    ...
    comparison_df.to_csv('results/spc_metropt3_benchmark_results.csv', index=False)

    md_table = f"""# Full SPC Benchmark Results: MetroPT-3 Air Compressor Dataset
...
| **Avg Latency per Chunk (ms)** | {shewhart_metrics['avg_latency_ms']:.4f} ms | {ewma_metrics['avg_latency_ms']:.4f} ms | {conv_metrics['avg_latency_ms']:.4f} ms | **{rbult_metrics['avg_latency_ms']:.4f} ms** | Low Latency Stream Processing |
"""
    with open('results/spc_metropt3_benchmark_comparison.md', 'w') as f:
        f.write(md_table)

    print("\nSaved benchmark summary to: results/spc_metropt3_benchmark_results.csv")
    print("Saved comparison table to:  results/spc_metropt3_benchmark_comparison.md")
...
if __name__ == '__main__':
    run_metropt3_spc_benchmark(chunk_size=1000)
