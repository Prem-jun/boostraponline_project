...
def parse_opt():
    ...
    parser.add_argument("--savename", type = str, default="result-fdist-statanal.csv", help = 'source for loading config file results')
    parser.add_argument("--outlier", action = 'store_true')
    opt = parser.parse_args()
    return opt

def run(source:str,savename:str,outlier:bool):
    ...
    for filename in filename_list:
        ...
        if not(ensure_directory_exists(figure_path)):
            break
        if outlier:
            file_re = os.path.join(folder_path,filename+'_re_outlier.pkl') # file results
        else:
        ...
        column = []
        idx = 0
        for size_run in ch_size:
            if size_run==50:
                
                idx = 0
            else:
                idx+=1
            if size_run in select_size: 
                # if size_run==50:
                #     idx =0
                # else:
                #     idx+=1
                for i in range(3):
                    ...
                    range_err.append(error_range[idx][i])
                    # nlearn_n.append(nlearn[idx][i][-1])
                    method.append(name[idx][i])
                # data = list(zip(range_err))
                df = pd.DataFrame(range_err, columns=method)
                if outlier:
                    df.to_csv(os.path.join(folder_path,savename+filename+'outlier'),index = 'False',float_format="%.4f")
                else:
                    df.to_csv(os.path.join(folder_path,savename+filename),index = 'False',float_format="%.4f")
