...
% =========================================================================
\section{Theoretical Foundations and 1D Range Estimation}\label{sec3_theory}
...
\subsection{Proposed Concept of RBULT}
\label{concept}
...
To construct the standard histogram $H = \{h_1, \dots, h_8\}$ without relying on standard normal assumptions, RBULT evaluates a candidate family of continuous distributions $\mathcal{P} = \{\text{Gamma}, \text{Weibull}, \text{Wald}, \text{Uniform}, \text{Rayleigh}, \text{Lognormal}, \dots\}$.

For an initial data chunk $c_1$, the optimal parameter vector $\hat{\boldsymbol{\theta}}$ for each candidate density $f(x; \boldsymbol{\theta})$ is estimated via Maximum Likelihood Estimation (MLE):
\begin{equation}
\hat{\boldsymbol{\theta}} = \arg\max_{\boldsymbol{\theta}} \sum_{j=1}^{|c_1|} \log f(x_{1,j}; \boldsymbol{\theta})
\end{equation}

The best-fitting distribution is selected by minimizing the Akaike Information Criterion (AIC) or Kolmogorov-Smirnov statistic. Once $\hat{\boldsymbol{\theta}}$ is selected, the expected theoretical element count $|h_k|$ for each bin $b_k$ ($k \in \{1, \dots, 8\}$) is computed via exact CDF integration:
\begin{equation}
