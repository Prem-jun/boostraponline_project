...
\item Integrates Algorithm 4 Z-score spike filtering with dynamic 10-candidate distribution MLE fitting to prevent control limit pollution.
\item Introduces a scale-free batch alarm threshold $C = \lceil 0.05\,k \rceil$ that is comparable across chunk sizes and dimensionalities, and reports both marginal and joint (all-dimension) coverage, the latter exposing a $25\%-71\%$ joint rate on $D=34$ streams that marginal coverage alone conceals.
...
\begin{itemize}
    ...
    \item A 4-stage statistical pipeline integrating Z-score spike filtering, lazy boundary expansion, 10-candidate distribution MLE fitting, and Bonferroni tail adjustment.
