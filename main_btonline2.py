...
def main(filewd = None,filename = None,nboost = None,boostkeep = None,minmax_boost = None,addnoise = False):
    if filewd is None:
        filewd = "./sim_data/"
    if filename is None:
    ...
    randomlist,feed_num,feed_ch,pop_max,pop_min,samp_max,samp_min = pickle.load(open(filewd+filename+".p","rb"))    
    if addnoise is True:
       feed_ch[7].append(pop_max+10)
       feed_ch[5].append(pop_min-10)
    samp_wh = []
    for i in feed_ch:
        samp_wh += i
    numbin = 8
    ...
    # max_right_end = samp_max[0]
    # min_left_end = samp_min[0]
    ...
    # for i in feed_ch:    
        ...
        if new_data_chunk_min < min_left_end:
            ...
            if len(end_bin_left) >= nboost:
                ...
                expand_min = True
                adjust_left_std = boostrap_v1.bootstrap_online(end_bin_left, "left",minmax_boost = minmax_boost)
                if min_left_end >= adjust_left_std:
                    min_left_end = adjust_left_std
                    # end_bin_left.append(adjust_left_std)
                else:
                    min_left_end = (min_left_end+adjust_left_std)/2
                # else:
        ...
        if new_data_chunk_max > max_right_end:
            ...
            if len(end_bin_right) >= nboost:
                ...
                # end_bin_right_tmp = copy.deepcopy(end_bin_right)
                # end_bin_right_tmp.append(new_data_chunk_max)
                end_bin_right.append(new_data_chunk_max)
                adjust_right_std = boostrap_v1.bootstrap_online(end_bin_right, "right",minmax_boost = minmax_boost)
                expand_max = True
                if max_right_end <= adjust_right_std:
                    
                    max_right_end = adjust_right_std
                    # end_bin_right.append(adjust_right_std)
                else:
                    max_right_end = (max_right_end+ adjust_right_std)/2
                # else:
        ...
        end_bin_left_tmp = end_bin_left
        end_bin_right_tmp = end_bin_right
        end_bin_left = []
        end_bin_right = []
        new_data_chunk = new_data_chunk + end_bin_left_tmp + end_bin_right_tmp
    ...
    print(f"# experiment data: {feed_num}", )
    ...
    # samp_wh = []
    # samp_wh = [samp_wh.append(i) for s in feed_ch]
    expand_left_min, expand_right_max = boostrap_v1.bootstrap_whole_data(input_data = samp_wh,\
                                                                         minmax_boost = minmax_boost)
    print("\n====================================================\n")
    print("-----bootstrap whole data")
